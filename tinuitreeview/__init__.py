...
class TinUITreeItem:
    ...
    def __init__(self, text: str, back, te, sign=None, parent=None):
        self.text: str = text
        self.back = back          # canvas item id（背景）
        self.te = te              # canvas item id（文字）
        self.sign = sign          # canvas item id（折叠图标），叶节点为 None
...
class TinUITreeView:
    """
        add_node(text, parent=None)->TinUITreeItem
        remove_node(item)
        rename_node(item, new_text)
        get_selected()->TinUITreeItem | None
        close_all()
        open_all()
    """
    _ICON_OPEN   = "\ue96e"
    ...
    def __init__(
        ...
        self._BasicTinUI   = BasicTinUI
        self._TinUIString  = TinUIString

        self._box = BasicTinUI(master, bg=bg, width=width, height=height)
        self._box.place(x=12, y=12)
    ...
    def remove_node(self, item: TinUITreeItem):
        ...
        # 计算需要向上移动的高度
        bbox_list = [self._box.bbox(n.back) for n in to_delete
                     if self._box.bbox(n.back) is not None]
        if bbox_list:
        ...
        if first_idx is not None:
            ...
            # 找到删除后第一个需要上移的 back
            for i, back in enumerate(remaining_backs):
...
if __name__ == "__main__":
    ...
    tree = TinUITreeView(tinui, (50, 50), command=lambda path: print("选中路径:", path))
